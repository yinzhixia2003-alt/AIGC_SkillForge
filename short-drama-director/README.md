...
把中文短剧、AI 漫剧、影视或网文改编剧本转换为具有导演决策优先级、连续时间码、复杂调度、POV与轴线控制，并可进入AI视频生产的导演分镜。
...
- 按“剧情动机→主要信息→空间连续→表演→节奏→电影化→生成稳定性”解决规则冲突
- 判断合镜与拆镜，建立 2-4 镜的小镜头组
- 允许多人物、多动作和前中后景调度，优先使用走位、声音和焦点转移控制观看重点
- 管理POV观察者位置、视线、认知边界和返回人物反应的链条
...
核心方法已根据仓库维护者提供的《导演分镜脚本生成 Skill（短剧弹性拆镜·导演决策系统版 V5.0）》升级，并提炼为本技能所需的决策优先级、复杂调度、POV、连续性、对白、转场和输出规范。原始方法全文不随开源包发布。
